...
/**
 * @file
 * Contains Drupal\config_refresh\ConfigRefreshManager
 */
  ...
  public function refreshAsBatch($module_name, $config_type) {
    ...
    if ($module_name == 'all') {
      foreach ($modules as $module) {
        $operations[] = array(array($class_name, 'updateConfigEntity'), array($module, $entity_storage, $config_entity_type));
        // $this->updateConfigEntity($module, $entity_storage, $config_entity_type);
      }
    }
    elseif (isset($modules[$module_name])) {
      $operations[] = array(array($class_name, 'updateConfigEntity'), array($modules[$module_name], $entity_storage, $config_entity_type));
